    ...
    <div class="panel-grid">
      <div class="viz-card">
        ...
        <div class="viz-surface">
          <svg class="viz-svg" :viewBox="schem.viewBox" preserveAspectRatio="xMidYMid meet" role="img" :aria-label="$t('fork_side_panel_title')">
            <defs>
            ...
            <!-- 立柱與底盤交接橫線 -->
            <line
              :x1="schem.bodyJoint.x1"
              :y1="schem.bodyJoint.y"
              :x2="schem.bodyJoint.x2"
              :y2="schem.bodyJoint.y"
              class="schem-body-joint"
            />

            <!-- Fork + carriage (Morandi blue): carriage fixed on mast; fork tine slides -->
            <g :transform="`translate(0 ${schem.fork.translateY})`">
              ...
              <!-- Thin trigger plate at sliding fork root (moves with fork.x) -->
              <rect
                v-if="isHorizonEnabled"
                :x="schem.triggerPlate.x"
                :y="schem.triggerPlate.y"
                :width="schem.triggerPlate.w"
                :height="schem.triggerPlate.h"
                class="schem-trigger-plate"
                rx="0.5"
              />

              <!-- Horizon sensors fixed at root; labels stagger below with elbow leaders -->
              <g v-if="isHorizonEnabled" class="sensors">
                <g v-for="mark in schem.horizonSensorMarks" :key="'h-' + mark.key">
                  <circle :cx="mark.x" :cy="mark.y" :r="schem.sensorR" :class="sensorDotClass(mark.isOn)" filter="url(#softGlow)" />
                  <path v-if="mark.leader" :d="mark.leader" class="sensor-leader" />
                  <text :x="mark.labelX" :y="mark.labelY" class="sensor-text sensor-text-center">{{ $t(mark.labelKey) }}</text>
                </g>
              </g>
            </g>
            ...
            <g class="sensors">
              <g v-for="mark in schem.verticalSensorMarks" :key="mark.key">
                <circle :cx="mark.x" :cy="mark.y" :r="schem.sensorR" :class="sensorDotClass(mark.isOn)" filter="url(#softGlow)" />
                <path v-if="mark.leader" :d="mark.leader" class="sensor-leader" />
                <text :x="mark.labelX" :y="mark.labelY" class="sensor-text sensor-text-left">{{ $t(mark.labelKey) }}</text>
              </g>
            </g>
            ...
            <g v-if="schem.heightDim.visible" class="height-dim" aria-hidden="true">
              <!-- Extension ticks -->
              <line
                :x1="schem.heightDim.tickX1"
                :y1="schem.heightDim.yTop"
                :x2="schem.heightDim.tickX2"
                :y2="schem.heightDim.yTop"
                class="height-dim-tick"
              />
              <line
                :x1="schem.heightDim.tickX1"
                :y1="schem.heightDim.yBot"
                :x2="schem.heightDim.tickX2"
                :y2="schem.heightDim.yBot"
                class="height-dim-tick"
              />
              <!-- Vertical dimension line -->
              <line
                :x1="schem.heightDim.x"
                :y1="schem.heightDim.yTop"
                :x2="schem.heightDim.x"
                :y2="schem.heightDim.yBot"
                class="height-dim-line"
              />
              ...
              <!-- Height label -->
              <text
                :x="schem.heightDim.labelX"
                :y="schem.heightDim.labelY"
                class="sensor-text height-dim-label"
            ...
            <g v-if="schem.overhangDim.visible" class="overhang-dim" aria-hidden="true">
              <!-- Extension ticks -->
              <line
                :x1="schem.overhangDim.xLeft"
                :y1="schem.overhangDim.tickY1"
                :x2="schem.overhangDim.xLeft"
                :y2="schem.overhangDim.tickY2"
                class="height-dim-tick"
              />
              <line
                :x1="schem.overhangDim.xRight"
                :y1="schem.overhangDim.tickY1"
                :x2="schem.overhangDim.xRight"
                :y2="schem.overhangDim.tickY2"
                class="height-dim-tick"
              />
              ...
              <line
                :x1="schem.overhangDim.xLeft"
                :y1="schem.overhangDim.y"
                :x2="schem.overhangDim.xRight"
                :y2="schem.overhangDim.y"
                class="height-dim-line"
              />
              <!-- Arrowheads (filled triangles) -->
              ...
              <!-- Overhang label -->
              <text
                :x="schem.overhangDim.labelX"
                :y="schem.overhangDim.labelY"
                class="sensor-text height-dim-label overhang-dim-label"
              >{{ formatNum(schem.overhangDim.cm) }} {{ $t('fork_viz_cm') }}</text>
            </g>
          </svg>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    schem() {
      ...
      const padX = 44
      const padY = 22
      const extendOverallForScale = Number(this.forkSettings?.VehielLengthWitchForkArmExtend ?? (vLen + 15))
      const drawLen = Math.max(vLen, Number.isFinite(extendOverallForScale) ? extendOverallForScale : vLen)
      ...
      const mast = { x: padX + 24, y: padY, w: mastW, h: mastH }
      ...
      const carriageW = Math.max(mastW * 1.6, 28)
      const carriageH = Math.max(40, Math.min(70, mastH * 0.2))
      // 滑架左側貼齊門架右緣（略咬入），避免與門架外框雙線重疊
      const carriageX = mast.x + mast.w - 3
      const carriage = { x: carriageX, y: -(carriageH / 2), w: carriageW, h: carriageH }

      // 縮回時牙叉前端對齊車身最前緣；伸出才超出車頭
      const bodyFrontX = base.x + base.w
      const forkStartX = carriage.x + carriage.w
      const retractForkLen = Math.max(24, bodyFrontX - forkStartX)
      ...
      const extendExtraCm = Math.max(0, (Number.isFinite(extendOverallCm) ? extendOverallCm : vLen) - vLen)
      const forkExtra = extendExtraCm * scale
      // Fixed physical fork length; slide position instead of stretching length
      const forkLen = retractForkLen + (this.isHorizonEnabled ? forkExtra : 0)
      const forkLenMax = forkLen
      // Tip travels bodyFrontX → bodyFrontX+forkExtra; nest into carriage when retracted
      const forkSlide = this.isHorizonEnabled ? forkExtra * this.horizonRatio : 0
      const forkX = (bodyFrontX + forkSlide) - forkLen // = forkStartX + forkExtra*(ratio-1) when horizon on
      const fork = { x: forkX, y: -(forkThickness / 2), w: forkLen, h: forkThickness, translateY: forkY }

      // Carriage fixed on mast; fork tine is a separate sliding bar (telescoping look)
      const cL = carriage.x
      ...
      const forkTinePath = [
        `M ${fL} ${fT}`,
        `L ${fTip - tipR} ${fT}`,
        `Q ${fTip} ${fT} ${fTip} ${fT + tipR}`,
        `L ${fTip} ${fB - tipR}`,
        `Q ${fTip} ${fB} ${fTip - tipR} ${fB}`,
        `L ${fL} ${fB}`,
        'Z'
      ].join(' ')

      const forkCfg = this.forkSettings
      const downPoseCm = forkCfg?.DownlimitPose ?? 0
      const upPoseCm = forkCfg?.UplimitPose ?? 0
      const homePoseCm = forkCfg?.HomePoseUseStandyPose === true ? (forkCfg?.StandbyPose ?? downPoseCm) : downPoseCm

      const verticalSensors = {
        up: { x: mast.x + mast.w / 2, y: yAtHeight(upPoseCm) },
        home: { x: mast.x + mast.w / 2, y: yAtHeight(homePoseCm) },
        down: { x: mast.x + mast.w / 2, y: yAtHeight(downPoseCm) }
      }

      // 近距標記用引線錯開標籤，避免 Home / 下極限文字重疊
      const sensorR = Math.max(3.5, Math.min(4.5, mastW * 0.22))
      const rawMarks = [
      ...
      // Horizon sensors fixed at fork-root zone (carriage junction), NOT along sliding tip.
      // Trigger plate at fork.x travels rootRetractX → rootExtendX; sensors sit on that strip.
      const rootTravel = this.isHorizonEnabled ? Math.max(forkExtra, 0) : 0
      const rootRetractX = forkStartX - rootTravel
      const rootExtendX = forkStartX
      const rootSpan = Math.max(rootTravel, 40)
      const rootCenterX = (rootRetractX + rootExtendX) / 2
      const sensorYBelow = Math.max(forkThickness / 2 + 14, 22)
      const horizonSensors = {
        retract: { x: rootCenterX - rootSpan / 2, y: sensorYBelow },
        home: { x: rootCenterX, y: sensorYBelow },
        extend: { x: rootCenterX + rootSpan / 2, y: sensorYBelow }
      }

      // 近距水平標記用引線錯開標籤，避免 縮回極限 / Home / 伸出極限 重疊
      const horizonRawMarks = [
        ...
        { key: 'home', labelKey: 'fork_viz_home', x: horizonSensors.home.x, y: horizonSensors.home.y, isOn: this.horizonHomeOn },
        { key: 'extend', labelKey: 'fork_viz_extend_limit', x: horizonSensors.extend.x, y: horizonSensors.extend.y, isOn: this.horizonExtendOn }
      ].sort((a, b) => a.x - b.x)

      const hMinGap = Math.max(58, sensorR * 8.5)
      ...
      // 若右側被往外擠過頭，整組往左收一點（仍盡量對齊 sensor 帶）
      const hMaxX = rootExtendX + Math.max(rootSpan, 40) + 24
      if (labelXs[labelXs.length - 1] > hMaxX) {
        const shift = labelXs[labelXs.length - 1] - hMaxX
        for (let i = 0; i < labelXs.length; i++) labelXs[i] -= shift
      }

      const labelBaseY = sensorYBelow + sensorR + 26
      const yStep = 14
      const horizonSensorMarks = this.isHorizonEnabled
        ? horizonRawMarks.map((m, i) => {
            const labelX = labelXs[i]
            // 交替下移一階，寬中文標籤再多一點垂直間距
            const labelY = labelBaseY + (i % 2) * yStep
            const elbowY = sensorYBelow + sensorR + 8
            const leader = `M ${m.x} ${m.y + sensorR + 1} L ${m.x} ${elbowY} L ${labelX} ${elbowY} L ${labelX} ${labelY - 8}`
            return {
              ...m,
              labelX,
              labelY,
      ...
      // Thin vertical plate on sliding fork root (nearest carriage), translates with fork.x
      const plateW = Math.max(3, Math.min(5, forkThickness * 0.32))
      const plateH = Math.max(forkThickness + 18, 30)
      const triggerPlate = {
        x: fork.x - plateW * 0.35,
        y: -plateH / 2,
        w: plateW,
        h: plateH
      }
      ...
      const ohMinGap = 14
      const ohVisible = ohGap >= ohMinGap
      const ohY = forkY - forkThickness / 2 - 16
      const overhangCm = Math.max(0, (fTip - bodyFrontX) / scale)
      const overhangDim = {
        visible: ohVisible,
        xLeft: ohXLeft,
        xRight: ohXRight,
        ...
        labelX: (ohXLeft + ohXRight) / 2,
        labelY: ohY - 8
      }

      // Fit viewBox to full fork travel so chassis scale stays constant while tip animates
      ...
      const contentRight = Math.max(
        base.x + base.w,
        forkStartX + forkLenMax + horizonLabelExtra,
      ...
      const horizonLabelPad = this.isHorizonEnabled && horizonSensorMarks.length
        ? Math.max(...horizonSensorMarks.map((m) => m.labelY)) + 10
      ...
      const vbMinY = Math.min(0, Math.floor(overhangTop))
