<template>
  <div class="battery d-flex flex-row" :style="'height:' + bHeight">
    <div>
      <i @click="RechargeCircuitControl" v-if="battery_status.IsCharging" style="color:limegreen"
        class="bi bi-battery-charging"></i>
      <i @click="RechargeCircuitControl" v-else :class="'bi bi-battery-full'"
        :style="{ color: IsLowBatteryLevel ? 'red' : 'black' }"></i>
    </div>
    <b-progress class="flex-fill h-100" :max="max" animated>
      <b-progress-bar animated v-bind:class="battery_state_bg" :value="battery_level"
        :label="battery_level <= 10 ? '' : `${battery_level}%${battery_status.IsCharging ? '(充電中)' : ''}`" v-bind:style="{
          fontSize: bHeight == '1rem' ? '12px' : '16px'
        }"></b-progress-bar>
    </b-progress>
    <div class="bat_level_show_when_low" v-if="battery_level <= 10" v-bind:style="{
      fontSize: bHeight == '1rem' ? '12px' : '16px'
