<template>
  <div class="home h-100">
    <div v-if="buildCstDataDialog.show">
      <el-dialog v-loading="buildCstDataDialog.loading" v-model="buildCstDataDialog.show" :modal="false" draggable title="貨物帳籍建立">
        ...
        <transition name="el-zoom-in-top">
          <div class="side h-100">
            <div class="opt-buttons px-1 py-1 d-flex flex-column">
              <b-button :disabled="back_end_server_err || VMSData.IsSystemIniting" @click="AGVInitialize()" class="mb-1 p-2" v-bind:class="VMSData.SubState == '' || (!VMSData.IsInitialized && VMSData.SubState != 'Initializing') ? 'down' : VMSData.SubState.toLowerCase()" block>
                <b v-if="VMSData.SubState != 'Initializing'">{{ $t('initialize') }}</b>
                <b v-else>{{ $t('initializing') }}</b>
              </b-button>
              <b-button :disabled="back_end_server_err" @click="AGVResetAlarm()" class="mb-1 p-2 border" block :variant="alarmResetBtnVariant">
                <b>{{ $t('reset_alarm') }}</b>
              </b-button>
              <b-button :disabled="back_end_server_err" @click="AGVBuzzerOff()" variant="light" class="mb-1 p-2 border" block>
                <b>
                  <i v-if="IsBuzzerMute" class="bi bi-volume-mute-fill"></i> {{ $t('buzzer_off') }} </b>
              </b-button>
              <b-button v-if="VMSData.Agv_Type != 2" :disabled="back_end_server_err" @click="ShowRemoveCstDialog()" variant="light" class="w-100 mb-1 p-2 border" v-bind:class="rm_cst_btn_class_bind" block>
                <b>{{ $t('cst-remove') }}</b>
              </b-button>
              <el-badge v-if="VMSData.Agv_Type != 2" @click="ShowRemoveCstDialog()" style="cursor:pointer;position: relative;bottom: 25px;left: 182px;" :value="cargo_status_text">
                <div @click="ShowRemoveCstDialog()" v-if="VMSData.Agv_Type != 2"></div>
              </el-badge>
              <b-button :disabled="back_end_server_err" :variant="IsLogin ? 'danger' : 'light'" class="mb-1 p-2 border" block @click="ShowLogin()">
            ...
            <div class="modes border rounded m-1 p-3 py-1 px-3 text-start">
              <div class="d-flex flex-row">
                ...
                <el-switch v-model="IsOnlineMode" @click.prevent="OnlineModeSwitchHandle()" :disabled="back_end_server_err || VMSData.IsSystemIniting" width="75" size="large" inline-prompt inactive-text="Offline" active-text="Online" active-color="rgb(13, 110, 253)"
                  inactive-color="rgb(220, 53, 69)"></el-switch>
              </div>
              <div class="d-flex flex-row">
                <div class="mode-item-label py-2">Auto Mode</div>
                <el-switch v-model="IsAutoMode" @click.stop.prevent="AutoModeSwitchHandle()" :disabled="back_end_server_err || VMSData.IsSystemIniting" width="75" size="large" inline-prompt inactive-text="Manual" active-text="Auto" active-color="rgb(13, 110, 253)"
                  inactive-color="rgb(220, 53, 69)"></el-switch>
              </div>
            </div>
            <el-divider style="margin:auto"></el-divider>
            ...
            <div class="coordination border rounded m-1 p-3 py-1">
              <div v-if="isBrakeSwitchRelease && !Is_TSMC_MiniAGV" class="w-100 d-flex">
                <div>
                  ...
                  <el-tag style="width:100px;height: 85px;font-size: 58px;" class="" :type="VMSData.BCR_State_MoveBase.tagID == 0 ? 'danger' : 'success'">{{ VMSData.BCR_State_MoveBase.tagID }}</el-tag>
                </div>
              ...
              <div v-else class="py-1 d-flex justify-content-center">
                <div class="m-1" v-if="!Is_TSMC_MiniAGV">
                  ...
                  <b-form-input size="sm" style="width:70px" disabled v-model="VMSData.BCR_State_MoveBase.tagID" :state="VMSData.BCR_State_MoveBase.tagID > 0"></b-form-input>
                </div>
                ...
                <div class="m-1">
                  ...
                  <b-form-input size="sm" style="width:120px" disabled :state="VMSData.LocStatus == 10" v-model="Coordination"></b-form-input>
                </div>
              </div>
