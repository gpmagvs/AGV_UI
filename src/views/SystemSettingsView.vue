  ...
  <div class="sys-setting">
    <el-drawer v-model="drawer_show" size="95%" direction="rtl">
      ...
      <div
        ...
        style="position: absolute; width:98%;height: 100%;top:66px"
            ...
            <el-scrollbar>
              <div class="menu-items">
                <div
                  ...
                  :key="index"
                  class="menu-item"
                  :class="{ active: selected_tab === item.index }"
          ...
          <div class="settings-content">
            <div v-if="selected_tab === '0'" class="tabpage border p-2">
              <el-form :model="settings" label-width="250" label-position="left">
                ...
                <el-form-item v-if="!IsInspectionAGV" label="初始化時有帳無料自動清帳">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.Auto_Cleaer_CST_ID_Data_When_Has_Data_But_NO_Cargo"
                  ></el-switch>
                </el-form-item>
                <el-form-item v-if="!IsInspectionAGV" label="初始化時有料無帳自動建帳">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.Auto_Read_CST_ID_When_No_Data_But_Has_Cargo"
                  ></el-switch>
                </el-form-item>
            ...
            <div v-if="selected_tab === '1'" class="tabpage border p-2">
              <el-form label-position="left" label-width="210">
                <el-form-item
                  v-if="settings.UI != undefined && settings.UI.IsQuicklyActionFooterDisplay != undefined"
                  label="快速動作功能列"
                >
                  <el-switch
                    v-model="settings.UI.IsQuicklyActionFooterDisplay"
                    @change="HandleParamChanged"
                    size="small"
                  ></el-switch>
                </el-form-item>
                ...
                >
                  <el-switch
                    v-model="settings.UI.CstReaderSwitchDisplayWhenNotLogin"
                    @change="HandleParamChanged"
                    size="small"
                  ></el-switch>
                </el-form-item>
              ...
              <div class="text-start w-100">
                <el-form :model="settings" label-width="210" label-position="left">
                  ...
                  <el-form-item label="左方雷射 Bypass">
                    <el-switch
                      v-model="settings.SensorBypass.LeftSideLaserBypass"
                      @change="HandleParamChanged"
                    ></el-switch>
                  </el-form-item>
                  <el-form-item label="右方雷射 Bypass">
                    <el-switch
                      v-model="settings.SensorBypass.RightSideLaserBypass"
                      @change="HandleParamChanged"
                    ></el-switch>
                  </el-form-item>
                  ...
                  <el-form-item label="啟用">
                    <el-switch
                      @change="HandleParamChanged"
                      v-model="settings.ImpactDetection.Enabled"
                    ></el-switch>
                  </el-form-item>
                  ...
                  <el-form-item label="閥值">
                    <el-input-number
                      ...
                      :max="4"
                      @change="HandleParamChanged"
                      v-model="settings.ImpactDetection.ThresHold"
                    ></el-input-number>
                  ...
                  <el-form-item label="啟用">
                    <el-switch
                      @change="HandleParamChanged"
                      v-model="settings.ImpactDetection.PitchErrorDetection"
                    ></el-switch>
                  </el-form-item>
                  <el-form-item label="異常檢出警報等級">
                    <el-select
                  ...
                  <el-form-item label="閥值">
                    <el-input-number
                      ...
                      :max="9.8"
                      @change="HandleParamChanged"
                      v-model="settings.ImpactDetection.PitchErrorThresHold"
                    ></el-input-number>
                    <span class="mx-2">G</span>
                  </el-form-item>
              ...
              <el-form :model="settings" label-width="250" label-position="left">
                <el-form-item label="等待充電開始時間(秒)">
                  <el-input-number
                    @change="HandleParamChanged"
                    size="small"
                    v-model="settings.BatteryModule.WaitChargeStartDelayTimeWhenReachChargeTaskFinish"
                  ></el-input-number>
                </el-form-item>
                <el-form-item label="斷開充電回路電壓閥值(mV)">
                  <el-input-number
                    @change="HandleParamChanged"
                    size="small"
                    v-model="settings.BatteryModule.CutOffChargeRelayVoltageThreshodlval"
                  ></el-input-number>
                </el-form-item>
                <el-form-item label="僅電量低於閥值才開啟充電迴路">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.BatteryModule.ChargeWhenLevelLowerThanThreshold"
                  ></el-switch>
                </el-form-item>
                <el-form-item label="充電迴路開啟閥值">
                  <el-input-number
                    ...
                    @change="HandleParamChanged"
                    v-model="settings.BatteryModule.ChargeLevelThreshold"
                  ></el-input-number>
                </el-form-item>
              </el-form>
            </div>
            ...
            <div v-if="selected_tab === '5'" class="tabpage border p-2">
              <el-form :model="settings" label-width="280" label-position="left">
                ...
                <el-form-item label="取貨時發生貨物傾斜時可暫停進行排除">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.LDULDParams.MaunalCheckAndResumableWhenUnloadButCargoBias"
                  ></el-switch>
                </el-form-item>
                <div class="text-start w-100 border-bottom">
                  <b>車頭設備內產品預檢知</b>
              ...
              <el-form label-position="left" label-width="210">
                <el-form-item label="行程上極限(cm)">
                  <el-input-number
                    ...
                    @change="HandleParamChanged"
                    v-model="settings.ForkAGV.UplimitPose"
                  ></el-input-number>
                </el-form-item>
                <el-form-item label="行程下極限(cm)">
                  <el-input-number
                    ...
                    :max="100"
                    @change="HandleParamChanged"
                    v-model="settings.ForkAGV.DownlimitPose"
                  ></el-input-number>
                </el-form-item>
                ...
                <el-form-item label="牙叉已在Home位置不必初始化">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.ForkNoInitializeWhenPoseIsHome"
                  ></el-switch>
                </el-form-item>
                <el-form-item label="退出設備時不等待牙叉縮回完成">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.ForkAGV.NoWaitForkArmFinishAndMoveOutInWorkStation"
                  ></el-switch>
                </el-form-item>
                <el-form-item label="退出設備後Z軸同步回Home">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.ForkAGV.NoWaitParkingFinishAndForkGoHomeWhenBackToSecondary"
                  ></el-switch>
                </el-form-item>
                <el-form-item label="取貨動作牙叉下降時同步拍照">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.ForkAGV.TriggerCstReaderWhenUnloadBackToEntryPointAndReachTag"
                  ></el-switch>
                </el-form-item>
                <el-form-item v-if="false" label="退出充電站後Z軸同步回Home">
                  <el-switch
                    @change="HandleParamChanged"
                    v-model="settings.ForkAGV.NoWaitParkingFinishAndForkGoHomeWhenBackToSecondaryAtChargeStation"
                  ></el-switch>
                </el-form-item>
                <el-form-item label="手動操作移動速度(%)">
                  <el-input-number
                    ...
                    :max="1"
                    @change="HandleParamChanged"
                    v-model="settings.ForkAGV.ManualModeOperationSpeed.MoveToPoseSpeed"
                  ></el-input-number>
                </el-form-item>
              </el-form>
            </div>
                  ...
                  <el-row>
                    ...
                    <el-col :span="10">
                      <el-input
                        @change="HandleParamChanged"
                        size="small"
                        v-model="settings.Connections.AGVS.IP"
                      ></el-input>
                    </el-col>
                    ...
                    <el-col :span="5">
                      <el-input-number
                        @change="HandleParamChanged"
                        size="small"
                        v-model="settings.Connections.AGVS.Port"
                      ></el-input-number>
                    </el-col>
                  </el-row>
                </el-form-item>
            ...
            <div v-if="selected_tab === '11'" class="tabpage border p-2">
              <EQHandshakeConfiguration
                :SyncFromAGVS="settings.SyncEQInfoFromAGVS"
            ...
            <div v-if="selected_tab === '13'" class="tabpage border p-2">
              ...
              <el-form label-position="left" label-width="250">
                <el-form-item label="無貨行駛異常時自動重置並上線">
                  <el-switch
                    v-model="settings.Advance.AutoInitAndOnlineWhenMoveWithoutCargo"
                    @change="HandleParamChanged"
                    size="small"
                  ></el-switch>
                </el-form-item>
                <el-form-item label="有貨行駛異常時自動重置並上線">
                  <el-switch
                    v-model="settings.Advance.AutoInitAndOnlineWhenMoveWithCargo"
                    @change="HandleParamChanged"
                    size="small"
                  ></el-switch>
                </el-form-item>
                <el-form-item label="禁止自動重置的異常">
                  <el-select
                    v-model="forbidden_auto_reset_alarm_codes"
                    @change="HandleForbiddenAutoResetAlarmCodesChanged"
                    size="small"
                    multiple
                    filterable
                  >
                    <el-option
                      v-for="alarm in alarm_table"
                      :key="alarm.Code"
                      :label="`${alarm.Code} - ${alarm.CN}`"
                      :value="alarm.Code"
                    ></el-option>
                  </el-select>
                </el-form-item>
